...
static int mtk_capture_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if (mPrepareDone == false) {
		...
		/* TCT: UL1 data source = Ext I2S ADC IN (ES7210), same as stock
		 * (stock AFE_I2S_CON2 = rate<<8|fmt<<3|enable at capture) */
		{
			struct audio_digital_i2s ext_adc_attr;

			memset(&ext_adc_attr, 0, sizeof(ext_adc_attr));
			ext_adc_attr.mI2S_SAMPLERATE = substream->runtime->rate;
			ext_adc_attr.mI2S_FMT = Soc_Aud_I2S_FORMAT_I2S;
			ext_adc_attr.mI2S_WLEN = Soc_Aud_I2S_WLEN_WLEN_16BITS;
			SetExtI2SAdcIn(&ext_adc_attr);
			SetExtI2SAdcInEnable(true);
			SetSampleRate(Soc_Aud_Digital_Block_MEM_I2S,
				      substream->runtime->rate);
			/* stock sets this: use external ADC (ES7210) */
			Afe_Set_Reg(AFE_ADDA_TOP_CON0, 0x1, 0x1);
			/* stock prepare programs the APLL dividers for the
			 * ES7210 clock chain:
			 * - CLK_AUDDIV_0 bit4 = 1 (div power, stock writes 1)
			 * - CLK_AUDDIV_1[23:16] = (VCO/128)/rate - 1
			 *   VCO = 0xac44000 for 11025/22050/88200/176400,
			 *         else 0xbb80000
			 * - enables the I2S_IN_2 memory block */
			{
				unsigned int vco = 0xbb80000;
				unsigned int div;

				if (substream->runtime->rate == 11025 ||
				    substream->runtime->rate == 22050 ||
				    substream->runtime->rate == 88200 ||
				    substream->runtime->rate == 176400)
					vco = 0xac44000;
				div = (vco >> 7) / substream->runtime->rate - 1;
				clksys_set_reg(CLK_AUDDIV_0, 0 << 4, 1 << 4);
				clksys_set_reg(CLK_AUDDIV_0, 1 << 10, 1 << 10);
				clksys_set_reg(CLK_AUDDIV_1, div << 16,
					       0xff0000);
			}
			SetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_IN_2,
					    true);
			/* stock enables the APLL2 (+tuner) for >48k capture */
			if (substream->runtime->rate > 48000) {
				pr_info("TCT capture prepare rate=%d\n",
					substream->runtime->rate);
				EnableALLbySampleRate(substream->runtime->rate);
				EnableAPLLTunerbySampleRate(
					substream->runtime->rate);
				AudDrv_APLL2Tuner_Clk_On();
			}
			tct_ext_adc_rate = substream->runtime->rate;
		}
	}
...
static int mtk_capture_alsa_stop(struct snd_pcm_substream *substream)
{
	...
	if (tct_ext_adc_rate > 48000) {
		DisableAPLLTunerbySampleRate(tct_ext_adc_rate);
		DisableALLbySampleRate(tct_ext_adc_rate);
	}
	tct_ext_adc_rate = 0;
