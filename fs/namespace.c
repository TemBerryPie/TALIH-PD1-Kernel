...
static int __maybe_unused susfs_mnt_alloc_id(struct mount *mnt)
{
	int res;
...
/* KernelSU backport: path_mount (from 5.15) */
	       ...
	       const char *type_page, unsigned long flags, void *data_page)
{
	...
	if (path->mnt->mnt_flags & MNT_UNBINDABLE)
		return -EINVAL;
...
/* KernelSU backport: path_umount (from 5.9) */
