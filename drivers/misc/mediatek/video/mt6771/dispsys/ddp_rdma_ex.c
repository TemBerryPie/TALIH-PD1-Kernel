...
unsigned int rdma_index(enum DISP_MODULE_ENUM module)
{
	...
	case DISP_MODULE_RDMA1:
		...
		break;
	default:
		DDPPR_ERR("invalid rdma module=%d\n", module);
		ASSERT(0);
		break;
	}
...
rdma_to_cmdq_event_nonsec_end(enum DISP_MODULE_ENUM module)
{
	...
	default:
		DDPPR_ERR("invalid rdma module=%d,rmda to cmdq event fail\n",
		       module);
		ASSERT(0);
		return DISP_MODULE_UNKNOWN;
	}
...
int rdma_start(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	/* Notify SPM RDMA dram request */
	if (module == DISP_MODULE_RDMA0) {
		DISP_REG_SET_FIELD(handle, FLD_SODI_RDMA0_REQ_MASKEN,
			DISP_REG_CONFIG_MMSYS_SODI_REQ_MASK, 0x1);
		DISP_REG_SET_FIELD(handle, FLD_SODI_RDMA0_REQ_MASKVAL,
			DISP_REG_CONFIG_MMSYS_SODI_REQ_MASK, 0x3);
	} else if (module == DISP_MODULE_RDMA1) {
		DISP_REG_SET_FIELD(handle, FLD_SODI_RDMA1_REQ_MASKEN,
			DISP_REG_CONFIG_MMSYS_SODI_REQ_MASK, 0x1);
		DISP_REG_SET_FIELD(handle, FLD_SODI_RDMA1_REQ_MASKVAL,
			DISP_REG_CONFIG_MMSYS_SODI_REQ_MASK, 0x3);
	}
...
int rdma_reset(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	while ((DISP_REG_GET(offset + DISP_REG_RDMA_GLOBAL_CON) & 0x700) ==
		...
		if (delay_cnt > 10000) {
			ret = -1;
			DDPPR_ERR("rdma%d_rst timeout, stage1! DISP_REG_RDMA_GLOBAL_CON=0x%x\n",
			       idx,
	...
	while ((DISP_REG_GET(offset + DISP_REG_RDMA_GLOBAL_CON) & 0x700) !=
		...
		ret = -1;
		DDPPR_ERR("rdma%d_rst timeout, stage2! DISP_REG_RDMA_GLOBAL_CON=0x%x\n",
		      ...
		      struct golden_setting_context *gsc)
{
	...
	Bpp = bpp / 8;
	if (!Bpp) {
		DDPPR_ERR("bpp is invalid, bpp=%d\n", bpp);
		return;
	}
	...
	if (temp < 0)
		...
		sodi_threshold_high = preultra_high > temp ?
			preultra_high : temp;
	...
	if (primary_display_is_video_mode()) {
		...
		DISP_REG_SET(handle, offset + DISP_REG_RDMA_MEM_GMC_SETTING_1,
			   ...
			   REG_FLD_VAL(FLD_RG_VALID_THRESHOLD_BLOCK_ULTRA, 1) |
			   REG_FLD_VAL(FLD_RG_VDE_BLOCK_ULTRA, 1));
	...
	dvfs_preultra_low = (preultra_low_us + fifo_off_dvfs) *
			consume_rate_div;

	dvfs_preultra_high = (preultra_high_us + fifo_off_dvfs) *
			consume_rate_div;
	...
	drs_enter = (preultra_low_us + fifo_off_drs_enter) * consume_rate_div;

	drs_leave = (preultra_low_us + fifo_off_drs_leave) * consume_rate_div;
...
void rdma_dump_golden_setting_reg(enum DISP_MODULE_ENUM module)
{
	unsigned int idx = rdma_index(module);
	unsigned long addr_offset = (DISP_RDMA_INDEX_OFFSET * idx);

	DDPDUMP("dump RDMA%d golden setting\n", idx);

	DDPDUMP("GMC_SETTING_0 [11:0]:%u [27:16]:%u\n",
	    DISP_REG_GET_FIELD(MEM_GMC_SETTING_0_FLD_PRE_ULTRA_THRESHOLD_LOW,
		addr_offset + DISP_REG_RDMA_MEM_GMC_SETTING_0),
	    DISP_REG_GET_FIELD(MEM_GMC_SETTING_0_FLD_PRE_ULTRA_THRESHOLD_HIGH,
		addr_offset + DISP_REG_RDMA_MEM_GMC_SETTING_0));
	DDPDUMP("GMC_SETTING_1 [11:0]:%u [27:16]:%u\n",
	    DISP_REG_GET_FIELD(MEM_GMC_SETTING_1_FLD_ULTRA_THRESHOLD_LOW,
		addr_offset + DISP_REG_RDMA_MEM_GMC_SETTING_1),
	...
	DDPDUMP("THRSHOLD_SODI [11:0]:%u [27:16]:%u\n",
		...
		addr_offset + DISP_REG_RDMA_THRESHOLD_FOR_SODI),
	    DISP_REG_GET_FIELD(RDMA_THRESHOLD_FOR_DVFS_FLD_HIGH,
		addr_offset + DISP_REG_RDMA_THRESHOLD_FOR_SODI));
	DDPDUMP("THRSHOLD_DVFS [11:0]:%u [27:16]:%u\n",
	    DISP_REG_GET_FIELD(RDMA_THRESHOLD_FOR_DVFS_FLD_LOW,
		addr_offset + DISP_REG_RDMA_THRESHOLD_FOR_DVFS),
	    DISP_REG_GET_FIELD(RDMA_THRESHOLD_FOR_DVFS_FLD_HIGH,
		addr_offset + DISP_REG_RDMA_THRESHOLD_FOR_DVFS));
	DDPDUMP("SRAM_SEL [0]:%u\n",
	    DISP_REG_GET(addr_offset + DISP_REG_RDMA_SRAM_SEL));
	DDPDUMP("DVFS_SETTING_PREULTRA [11:0]:%u [27:16]:%u\n",
	    DISP_REG_GET_FIELD(RDMA_THRESHOLD_FOR_DVFS_FLD_LOW,
		addr_offset + DISP_REG_RDMA_DVFS_SETTING_PRE),
	    DISP_REG_GET_FIELD(RDMA_THRESHOLD_FOR_DVFS_FLD_HIGH,
		addr_offset + DISP_REG_RDMA_DVFS_SETTING_PRE));
	DDPDUMP("DVFS_SETTING_ULTRA [11:0]:%u [27:16]:%u\n",
	    DISP_REG_GET_FIELD(RDMA_THRESHOLD_FOR_DVFS_FLD_LOW,
		addr_offset + DISP_REG_RDMA_DVFS_SETTING_ULTRA),
	    DISP_REG_GET_FIELD(RDMA_THRESHOLD_FOR_DVFS_FLD_HIGH,
		addr_offset + DISP_REG_RDMA_DVFS_SETTING_ULTRA));
	DDPDUMP("LEAVE_DRS_SETTING [11:0]:%u [27:16]:%u\n",
	    DISP_REG_GET_FIELD(RDMA_THRESHOLD_FOR_DVFS_FLD_LOW,
		addr_offset + DISP_REG_RDMA_LEAVE_DRS_SETTING),
	    DISP_REG_GET_FIELD(RDMA_THRESHOLD_FOR_DVFS_FLD_HIGH,
		addr_offset + DISP_REG_RDMA_LEAVE_DRS_SETTING));
	DDPDUMP("ENTER_DRS_SETTING [11:0]:%u [27:16]:%u\n",
	    DISP_REG_GET_FIELD(RDMA_THRESHOLD_FOR_DVFS_FLD_LOW,
		addr_offset + DISP_REG_RDMA_ENTER_DRS_SETTING),
	    DISP_REG_GET_FIELD(RDMA_THRESHOLD_FOR_DVFS_FLD_HIGH,
		addr_offset + DISP_REG_RDMA_ENTER_DRS_SETTING));
		       ...
		       unsigned int bpp)
{
	...
	if ((width > RDMA_MAX_WIDTH) || (height > RDMA_MAX_HEIGHT))
		DDPPR_ERR("RDMA input overflow, w=%d, h=%d, max_w=%d, max_h=%d\n",
	...
	if (input_is_yuv == 1 && output_is_yuv == 0) {
		...
		case 2:
			...
			break; /* BT709 */
		default:
			DDPPR_ERR("%s,un-recognized yuv_range=%d!\n",
				__func__, yuv_range);
			color_matrix = 4;
			break;
		}
...
void rdma_dump_reg(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("(0x100)0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_DBG_OUT + offset));
	DDPDUMP("(0x10c)0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_DBG_OUT1 + offset));
	DDPDUMP("(0x110)0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_DBG_OUT2 + offset));
	DDPDUMP("(0x114)0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_DBG_OUT3 + offset));
	DDPDUMP("(0x118)0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_DBG_OUT4 + offset));
	DDPDUMP("(0x11c)0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_DBG_OUT5 + offset));
...
void rdma_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("en=%d,mode:%s,smi_busy:%d,wh(%dx%d),pitch=%d,addr=0x%08x,fmt=%s,fifo_sz=%u,output_valid_threshold=%u,fifo_min=%d\n",
		...
		DISP_REG_GET(DISP_REG_RDMA_FIFO_LOG + offset));
	DDPDUMP("pos:in(%d,%d)out(%d,%d),bg(t%d,b%d,l%d,r%d),start=%lld ns,end=%lld ns\n",
		DISP_REG_GET(DISP_REG_RDMA_IN_P_CNT + offset),
		DISP_REG_GET(DISP_REG_RDMA_IN_LINE_CNT + offset),
		DISP_REG_GET(DISP_REG_RDMA_OUT_P_CNT + offset),
		DISP_REG_GET(DISP_REG_RDMA_OUT_LINE_CNT + offset),
			    ...
			    struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	/* bandwidth report */
	if (module == DISP_MODULE_RDMA0)
		DISP_SLOT_SET(handle, DISPSYS_SLOT_BASE, DISP_SLOT_RDMA0_BW,
			(unsigned int)rdma_bw);
