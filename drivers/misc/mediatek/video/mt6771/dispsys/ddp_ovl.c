			...
			struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	/* tb8788p1: the stock ddp_ovl writes per-layer non-zero values when
	 * disabling a layer's RDMA (0xbc/0xb8/0xa9 for layer0/1/2, decoded
	 * from the stock ovl_config_l). Writing 0 keeps the undocumented
	 * RDMA_CTRL control bits off; align with the stock values. */
	for (ovl_layer = 0; ovl_layer < ovl_layer_num(module); ovl_layer++) {
		static const unsigned int layer_off_val[] = {
			0xbc, 0xb8, 0xa9, 0xa9};

		layer_offset_rdma_ctrl = ovl_base + ovl_layer *
				OVL_LAYER_OFFSET;
		DISP_REG_SET(handle, DISP_REG_OVL_RDMA0_CTRL +
				layer_offset_rdma_ctrl,
				layer_off_val[ovl_layer]);
	}
