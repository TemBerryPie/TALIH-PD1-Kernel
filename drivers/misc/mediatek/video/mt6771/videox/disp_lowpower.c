...
static struct golden_setting_context *__get_golden_setting_context(void)
{
	...
	/* tb8788p1: DISP_OPT_FAKE_LCM_WIDTH/HEIGHT are never set in this BSP
	 * (default 0), so the golden-setting FIFO math below computed zero
	 * thresholds and the RDMA frame-tail underflowed (the flickering
	 * strip at the bottom). Use the real panel size instead. */
	if (!gs_ctx.dst_width || !gs_ctx.dst_height) {
		gs_ctx.dst_width = primary_display_get_width();
		gs_ctx.dst_height = primary_display_get_height();
	}
	...
	if (gs_ctx.dst_width == 1080 && gs_ctx.dst_height == 1920)
		...
		gs_ctx.hrt_magicnum = 4;
	/* tb8788p1: no 1600x2176 special case here — the stock kernel runs this
	 * panel with hrt_magicnum=0 and mmsys on the 400 MHz VENCPLL (dynamic
	 * switch disabled); see disp_helper.c. */
