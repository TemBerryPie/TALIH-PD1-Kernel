...
int disp_helper_get_option(enum DISP_HELPER_OPT option)
{
	...
	if (option >= DISP_OPT_NUM) {
		DISPPR_ERROR("%s: option invalid %d\n", __func__, option);
		return -1;
	}
...
void disp_helper_option_init(void)
{
	...
	disp_helper_set_option(DISP_OPT_IDLE_MGR, 0); /* TEST: disable idle mgr (DL) */
	...
	/* 3. cmd mode + vdo mode */
	/* tb8788p1: the stock kernel dumps show DYNAMIC_SWITCH_MMSYSCLK=0 and
	 * DYNAMIC_RDMA_GOLDEN_SETTING=1. With the dynamic switch enabled this
	 * BSP has no caller for primary_display_switch_mmsys_clk(), so MM_VENCPLL
	 * is never enabled and mmsys is pinned at 300 MHz; the RDMA/OVL FIFOs
	 * starve under load (severe frame underflow after wake, which breaks the
	 * TDDI panel sync -> persistent display corruption). Match the stock. */
	...
	/* smart layer OVL*/
	/* tb8788p1: keep the OVL ext layer OFF. With it on, the
	 * bottom-most layer (system bar, 1600x56 @ y2120) is placed in
	 * the extended layer and its output corrupts the bottom band of
	 * the screen in portrait (flicker + displacement, fixed by
	 * disabling ext layers; stock never puts a large layer there). */
