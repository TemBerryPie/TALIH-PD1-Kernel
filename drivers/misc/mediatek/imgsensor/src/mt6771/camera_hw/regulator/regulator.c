	...
	struct IMGSENSOR_HW_DEVICE_COMMON *pcommon)
{
	...
	for (idx = IMGSENSOR_SENSOR_IDX_MIN_NUM;
		...
		for (type = 0; type < REGULATOR_TYPE_MAX_NUM; type++) {
			...
			/* tb8788p1: the stock DT supplies are named "vcama"/
			 * "vcamd"/"vcamio" for the main sensor and "*_sub"
			 * for the sub (see the dtbo fragment@60 on
			 * kd_camera_hw1), not "cam%d_%s". */
			ret = snprintf(str_regulator_name,
				sizeof(str_regulator_name),
				"%s%s",
				regulator_control[type].pregulator_type,
				(idx == IMGSENSOR_SENSOR_IDX_MIN_NUM) ?
					"" : "_sub");
			if (ret < 0)
				return ret;
			...
			if (preg->pregulator[idx][type] == NULL)
				PK_PR_ERR("regulator[%d][%d]  %s fail!\n",
