...
static imgsensor_info_struct imgsensor_info = {
	...
	.pre = {
		.pclk = 479555000,				//record different mode's pclk
...
//fps 15
	.cap1 = {							//capture for PIP 15ps relative information, capture1 mode must use same framelength, linelength with Capture mode for shutter calculate
		.pclk = 479555000,				//record different mode's pclk
		.linelength  = 4896,				//record different mode's linelength
		.framelength = 6529,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width  = 4208,		//record different mode's width of grabwindow
		.grabwindow_height = 3120,		//record different mode's height of grabwindow
  ...
  .custom1 = {
		.pclk = 479555000,				//record different mode's pclk
		.linelength  = 4896,//5808,				//record different mode's linelength
		.framelength = 4081,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width  = 4208,		//record different mode's width of grabwindow
		.grabwindow_height = 3120,		//record different mode's height of grabwindow
  ...
  .custom2 = {
		...
		.framelength = 3289,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width  = 4208,		//record different mode's width of grabwindow
		.grabwindow_height = 3120,		//record different mode's height of grabwindow
  ...
  .custom3 = {
		...
		.framelength =3481, //3168,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
  ...
  .custom5 = {
	...
	.frame_time_delay_frame = 1,    //lizhiyong 20190719
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =
{
 ...
 { 4208, 3120,	  0,    0, 4208, 3120, 1280, 720,    0,	0, 1280,  720, 	 0, 0, 1280,  720}, //hight speed video, //hight speed video
...
/* */
	...
	printk("s5k3l6 set_shutter_frame_length shutter:%d, frame_length:%d, realtime_fps:%d, framelength =%d\n", shutter, frame_length, realtime_fps, imgsensor.frame_length);
}
//end 
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x0136,0x1A00);
	write_cmos_sensor(0x0304,0x0003);
	write_cmos_sensor(0x0306,0x0053);
	...
	write_cmos_sensor(0x0340,0x0CC1);
	...
	write_cmos_sensor(0x0820,0x023c);
	...
	write_cmos_sensor(0x3C38,0x0028);
...
static void capture_setting(kal_uint16 currefps)
{
    ...
    if (currefps == 300) {
        ...
        check_stream_is_off();
				...
				write_cmos_sensor(0x3C1E, 0x0000);
				write_cmos_sensor(0x030C, 0x0003);
				write_cmos_sensor(0x030E, 0x0045);
				...
				write_cmos_sensor(0x3938, 0x000C);
				write_cmos_sensor(0x0820, 0x04AC);
    	...
    	write_cmos_sensor(0x0100,0x0000);
			...
			write_cmos_sensor(0x0300, 0x0006);
			write_cmos_sensor(0x0342, 0x1320);
			write_cmos_sensor(0x0340, 0x0FF1);
			write_cmos_sensor(0x38C4, 0x0009);
			write_cmos_sensor(0x38D8, 0x002A);
			write_cmos_sensor(0x38DA, 0x000A);
			write_cmos_sensor(0x38DC, 0x000B);
    	...
    	write_cmos_sensor(0x0100,0x0000);
			...
			write_cmos_sensor(0x0300, 0x0006);
			write_cmos_sensor(0x0342, 0x1320);
			write_cmos_sensor(0x0340, 0x1981);
			write_cmos_sensor(0x38C4, 0x0009);
			write_cmos_sensor(0x38D8, 0x002A);
			write_cmos_sensor(0x38DA, 0x000A);
			write_cmos_sensor(0x38DC, 0x000B);
    ...
    //write_cmos_sensor_byte(0x3C67,0x10);    //dual cam sync delete by wxs for software sync
}
...
{
	LOG_INF("s5k3l6 custom1_setting:%d\n", currefps);
...
static kal_uint32 open(void)
{
    ...
    //update_3l6_eepromInfo(); //print eeprom info
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_CHECK_SENSOR_ID:
            ...
            break;
	#if 0
		case SENSOR_FEATURE_GET_CAM_INFO:
			*feature_para = 6; // 1:Darling; 2:Shinetech; 3:ChengXiang 4:TianShi 5:TXD 6:HongShi
			break;
	#endif
	case SENSOR_FEATURE_GET_PIXEL_RATE:
		...
		case SENSOR_FEATURE_GET_PDAF_INFO:
			...
			case SENSOR_FEATURE_GET_PDAF_DATA:
				read_eeprom_s5k3l6((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*       (feature_data+2)));
