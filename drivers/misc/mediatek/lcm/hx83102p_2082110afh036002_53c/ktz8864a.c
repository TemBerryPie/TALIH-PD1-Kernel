...
int ktz8864a_read_byte(unsigned char cmd, unsigned char *returnData)
{
	...
	/* tb8788p1: the ktz8864a has no dt node so the i2c client may never be
	 * probed; guard against a NULL client instead of panicking in
	 * i2c_master_send/recv (this crashes the device on resume). */
	if (!new_client) {
		if (returnData)
			*returnData = 0;
		return 0;
...
int ktz8864a_write_byte(unsigned char cmd, unsigned char writeData)
{
	...
	/* tb8788p1: guard NULL client (no dt node -> not probed) to avoid a
	 * resume-time panic in i2c_master_send. Panel power was already set
	 * up by LK, so skipping these writes is safe for wake-up. */
	if (!new_client)
		return 0;
