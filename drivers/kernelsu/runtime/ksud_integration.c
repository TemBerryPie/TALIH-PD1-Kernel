...
static inline long ksu_copy_from_user_nofault(void *dst, const void __user *src, unsigned long size)
{
    long ret = -EFAULT;

    pagefault_disable();
    if (likely(!__copy_from_user(dst, src, size)))
        ret = 0;
    pagefault_enable();
    return ret;
}

static inline long ksu_copy_to_user_nofault(void __user *dst, const void *src, unsigned long size)
{
    long ret = -EFAULT;

    pagefault_disable();
    if (likely(!__copy_to_user(dst, src, size)))
        ret = 0;
    pagefault_enable();
    return ret;
}
#define strncpy_from_user_nofault(dst, src, count) ksu_strncpy_from_user_nofault(dst, src, count)
#define copy_from_user_nofault(dst, src, size) ksu_copy_from_user_nofault(dst, src, size)
#define copy_to_user_nofault(dst, src, size) ksu_copy_to_user_nofault(dst, src, size)
#endif
...
static long (*orig_sys_read)(unsigned int fd, char __user *buf, size_t count);
static long ksu_sys_read(unsigned int fd, char __user *buf, size_t count)
{
    ksu_handle_sys_read(fd, &buf, &count);
    return orig_sys_read(fd, buf, count);
}

static long (*orig_sys_fstat)(unsigned int fd, struct stat __user *statbuf);
static long ksu_sys_fstat(unsigned int fd, struct stat __user *statbuf)
{
    bool is_rc = false;
    long ret;
