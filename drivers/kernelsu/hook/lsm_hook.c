...
/* 4.14: security_hook_heads uses list_head and there is no exported
 * runtime slot patching API. Hook the selinux_ops table directly.
 * SukiSU only uses the setprocattr hook on this codebase. */

static DEFINE_MUTEX(ksu_lsm_hook_lock);
static struct security_hook_heads *ksu_lsm_heads;
static struct ksu_lsm_hook *ksu_lsm_hook_entries[4];
static int ksu_lsm_hook_count;

static struct security_hook_heads *ksu_lsm_find_heads(void)
{
    if (ksu_lsm_heads)
        return ksu_lsm_heads;

    ksu_lsm_heads =
        (struct security_hook_heads *)find_kernel_symbol_exact("security_hook_heads");
    if (!ksu_lsm_heads) {
        pr_err("lsm_hook: security_hook_heads not found\n");
        return NULL;
    }

    pr_info("lsm_hook: security_hook_heads: 0x%lx\n", (long)ksu_lsm_heads);
    return ksu_lsm_heads;
}

static struct security_hook_list *ksu_lsm_find_selinux_entry(struct list_head *head)
{
    struct security_hook_list *p;

    list_for_each_entry(p, head, list) {
        if (p->lsm && !strcmp(p->lsm, "selinux"))
            return p;
    }
    return NULL;
}
...
int ksu_lsm_hook(struct ksu_lsm_hook *hook)
{
    ...
    if (strcmp(hook->head_name, "setprocattr")) {
        pr_err("lsm_hook: 4.14 path only supports setprocattr (got %s)\n",
               hook->head_name ?: "unknown");
        return -ENOSYS;
    }

    mutex_lock(&ksu_lsm_hook_lock);
    heads = ksu_lsm_find_heads();
    if (!heads) {
        ret = -ENOENT;
        goto out;
    }
    ...
    entry = ksu_lsm_find_selinux_entry(&heads->setprocattr);
    if (!entry) {
        pr_err("lsm_hook: selinux entry not found on setprocattr list\n");
        ret = -ENOENT;
        goto out;
    }

    hook->original = entry->hook.setprocattr;
    slot = (void **)&entry->hook.setprocattr;
    if (ksu_patch_text(slot, &hook->replacement, sizeof(hook->replacement),
                       KSU_PATCH_TEXT_FLUSH_DCACHE)) {
        pr_err("lsm_hook: patch setprocattr slot failed\n");
        ret = -EIO;
        goto out;
    }
    smp_wmb();

    ksu_lsm_hook_entries[ksu_lsm_hook_count++] = hook;
    hook->entry = entry;
    pr_info("lsm_hook: patched %s slot %px -> %px (orig %px)\n", hook->head_name,
            slot, hook->replacement, hook->original);
out:
...
void ksu_lsm_unhook(struct ksu_lsm_hook *hook)
{
    ...
    mutex_lock(&ksu_lsm_hook_lock);
    slot = (void **)&hook->entry->hook.setprocattr;
    if (ksu_patch_text(slot, &hook->original, sizeof(hook->original),
                       KSU_PATCH_TEXT_FLUSH_DCACHE))
        pr_err("lsm_hook: restore setprocattr slot failed\n");
    smp_wmb();

    for (i = 0; i < ksu_lsm_hook_count; i++) {
        if (ksu_lsm_hook_entries[i] == hook) {
...
{
    pr_info("lsm_hook: init (4.14 selinux_ops path)\n");
}
...
void __exit ksu_lsm_hook_exit(void)
{
    ...
    for (i = ksu_lsm_hook_count - 1; i >= 0; i--)
        ksu_lsm_unhook(ksu_lsm_hook_entries[i]);
}
