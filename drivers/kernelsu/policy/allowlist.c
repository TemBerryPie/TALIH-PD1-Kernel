...
bool ksu_uid_should_umount(uid_t uid)
{
    ...
    if (unlikely(uid == 1000)) {
        // system needs to see module mounts (e.g. microG vending as
        // the required package installer on GSI builds); umounting
        // here makes PMS crash with "exactly one installer" loop.
        return false;
    }
#ifdef CONFIG_KSU_DISABLE_POLICY
    return !__ksu_is_allow_uid(uid);
...
#else
                strncpy(domain, KSU_DEFAULT_SELINUX_DOMAIN, domain_len);
#endif
