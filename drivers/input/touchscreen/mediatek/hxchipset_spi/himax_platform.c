...
const struct of_device_id himax_match_table[] = {
	...
	{.compatible = "mediatek,spi_himax_touch" },
...
static int hx_panel_coords[4] = {0, 1600, 0, 2176};
static int hx_display_coords[4] = {0, 1600, 0, 2176};
...
/* tb8788p1 touch trace counters (flood-safe), see himax_platform.h */
struct himax_trace g_hxtrace;
EXPORT_SYMBOL(g_hxtrace);
		...
		uint32_t length, uint8_t toRetry)
{
	...
	/* tb8788p1 dynamic trace: count reads + snapshot the event read.
	 * No printk here (the touch IRQ storm floods/ratelimits printk);
	 * read the counters via /proc/android_touch/hxtrace after touching. */
	atomic_long_inc(&g_hxtrace.bus_rd);
	g_hxtrace.last_rd_cmd = command;
	g_hxtrace.last_rd_len = length;
	if (command == 0x30) {
		int cplen = length < 16 ? length : 16;

		atomic_long_inc(&g_hxtrace.bus_rd_ev);
		g_hxtrace.last_ev_len = length;
		g_hxtrace.last_ev_res = result;
		memcpy(g_hxtrace.last_ev, data, cplen);
	}
		...
		uint32_t length, uint8_t toRetry)
{
	...
	/* tb8788p1 dynamic trace: count writes (init config capture stays via
	 * the boot-time HXTR WR print; writes don't fire in the IRQ storm) */
	atomic_long_inc(&g_hxtrace.bus_wr);
	{
		static int wrlog;

		if (wrlog < 400 && length > 0) {
			wrlog++;
			pr_info("HXTR WR cmd=%02x d=%02x %02x %02x %02x len=%d\n",
				command, data[0], length > 1 ? data[1] : 0,
...
int himax_int_register_trigger(void)
{
	...
	/* tb8788p1: the HX83102E FW drives INT as an active-low LEVEL line
	 * (the stock driver registers IRQF_TRIGGER_LOW, and register 0x10007088
	 * reads 0x90 => level). Our probe read HX_INT_IS_EDGE=1 from the chip
	 * before it was fully up, which mis-registered the IRQ as edge and lost
	 * touch interrupts on a level line. Force LEVEL like the stock driver. */
...
int himax_ts_register_interrupt(void)
{
	...
	/* tb8788p1: the touch interrupt (EINT) lives on the SPI device node
	 * (spi_himax_touch), not on the tpd framework node; parse the IRQ from
	 * the SPI device's own of_node first, fall back to the tpd node. */
	if (hx_spi && hx_spi->dev.of_node &&
	    of_property_read_bool(hx_spi->dev.of_node, "interrupts")) {
		ts->hx_irq = irq_of_parse_and_map(hx_spi->dev.of_node, 0);
		I("himax_touch_irq from spi dev node = %d\n", ts->hx_irq);
	} else if (node) {
