...
/* ---- tb8788p1 touch trace dump node (/proc/android_touch/hxtrace) ------
 * Flood-safe diagnostic: the IRQ storm drowns printk, so the touch path is
 * counted with atomics (see himax_platform.c / himax_common.c) and dumped
 * here on demand. Usage on device:
 *   echo 0 > /proc/android_touch/hxtrace   # reset counters
 *   <touch the panel a few times>
 *   cat /proc/android_touch/hxtrace        # read what actually happened */
static struct proc_dir_entry *himax_proc_hxtrace_file;

static int himax_hxtrace_show(struct seq_file *s, void *v)
{
	int i;

	seq_puts(s, "== himax touch trace (counters since last reset) ==\n");
	seq_printf(s, "ts_work_enter : %ld   (threaded IRQ handler ran)\n",
		atomic_long_read(&g_hxtrace.ts_work_enter));

	seq_puts(s, "ts_path       :");
	for (i = 0; i < 4; i++) {
		const char *n = "";

		if (i == HX_REPORT_COORD)
			n = "(COORD)";
		else if (i == HX_REPORT_SMWP_EVENT)
			n = "(SMWP)";
		else if (i == HX_REPORT_COORD_RAWDATA)
			n = "(COORD_RAW)";
		seq_printf(s, " [%d]%s=%ld", i, n,
			atomic_long_read(&g_hxtrace.ts_path[i]));
	}
	seq_puts(s, "\n");

	seq_printf(s, "op_done       : %ld\n",
		atomic_long_read(&g_hxtrace.op_done));
	seq_printf(s, "get_data_fail : %ld\n",
		atomic_long_read(&g_hxtrace.get_fail));

	seq_printf(s, "bus_rd        : %ld   bus_wr: %ld\n",
		atomic_long_read(&g_hxtrace.bus_rd),
		atomic_long_read(&g_hxtrace.bus_wr));
	seq_printf(s, "bus_rd_ev(30) : %ld   (event-stack reads)\n",
		atomic_long_read(&g_hxtrace.bus_rd_ev));
	seq_printf(s, "last_rd       : cmd=0x%02x len=%d\n",
		g_hxtrace.last_rd_cmd & 0xff, g_hxtrace.last_rd_len);

	seq_printf(s, "cksum_ok      : %ld\n",
		atomic_long_read(&g_hxtrace.cksum_ok));
	seq_printf(s, "cksum_fail    : %ld\n",
		atomic_long_read(&g_hxtrace.cksum_fail));
	seq_printf(s, "cksum_allzero : %ld\n",
		atomic_long_read(&g_hxtrace.cksum_allzero));
	seq_printf(s, "last_cksum    : 0x%02x (mod 256; 0==pass)\n",
		g_hxtrace.last_cksum & 0xff);

	seq_printf(s, "last_ev       : len=%d res=%d data=",
		g_hxtrace.last_ev_len, g_hxtrace.last_ev_res);
	for (i = 0; i < 16; i++)
		seq_printf(s, "%02x", g_hxtrace.last_ev[i]);
	seq_puts(s, "\n");

	seq_puts(s, "hint: ts_work_enter>0 => handler ran; bus_rd_ev>0 => event read used cmd 0x30; cksum_fail/allzero explain empty FIFO\n");
	return 0;
}

static int himax_hxtrace_open(struct inode *inode, struct file *file)
{
	return single_open(file, himax_hxtrace_show, NULL);
}

static ssize_t himax_hxtrace_write(struct file *file, const char __user *buf,
		size_t count, loff_t *ppos)
{
	/* any write resets the counters */
	memset(&g_hxtrace, 0, sizeof(g_hxtrace));
	return count;
}

static const struct file_operations himax_proc_hxtrace_ops = {
	.owner = THIS_MODULE,
	.open = himax_hxtrace_open,
	.read = seq_read,
	.write = himax_hxtrace_write,
	.llseek = seq_lseek,
...
int himax_touch_proc_init(void)
{
	...
	/* tb8788p1 touch trace dump (diagnostic only; failure is non-fatal) */
	himax_proc_hxtrace_file = proc_create("hxtrace", 0644,
				himax_touch_proc_dir,
				&himax_proc_hxtrace_ops);
	if (himax_proc_hxtrace_file == NULL)
		E(" %s: proc hxtrace file create failed!\n", __func__);
...
void himax_touch_proc_deinit(void)
{
	...
	if (himax_proc_hxtrace_file)
		remove_proc_entry("hxtrace", himax_touch_proc_dir);
