...
/* tb8788p1: incell touch functions from himax_ic_incell_core.c; the variant
 * re-init (hx83102e_func_re_init) must not keep its standalone overrides for
 * this incell part. */
extern void himax_mcu_init_ic(void);
extern void himax_mcu_sense_on(uint8_t FlashMode);
extern bool himax_mcu_sense_off(bool check_en);
extern bool himax_mcu_read_event_stack(uint8_t *buf, uint8_t length);
...
static void hx83102e_chip_init(void)
{
	...
	/* tb8788p1: stock also sets CFG_TABLE_FLASH_ADDR in the e-variant
	 * (found via machine-code diff); keep parity. */
	(*kp_CFG_TABLE_FLASH_ADDR) = 0x10000;
...
static bool hx83102e_read_event_stack(uint8_t *buf, uint8_t length)
{
	...
	/* tb8788p1: the stock chip-specific read is a plain single bus read
	 * (verified by dumping the stock kernel's runtime code) — no AHB
	 * window toggling here. The 4-byte + 0xFF-tail frames seen earlier
	 * were artifacts of reading with IRQ disabled; the real issue was
	 * touch_info_size (68 vs 32), fixed via HX_FIX_TOUCH_INFO. */
...
static bool hx83102_chip_detect(void)
{
	...
	/* tb8788p1: hard-reset the touch here, matching the stock driver
	 * (_hx83102_init does pin_reset before sense_off/chip_detect). The
	 * earlier "keep LK scan" hack left the TDDI in a bad state. */
	hx83102_pin_reset();
