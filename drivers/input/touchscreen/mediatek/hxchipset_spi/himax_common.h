...
#define HIMAX_DRIVER_VER "2.0.0.68_ABCD1234_02"
...
/* tb8788p1: disable the exception-recovery re-flash loop; the touch runs
 * its downloaded firmware and must not be reset/reflashed repeatedly. */
...
/* tb8788p1: the stock kernel has no zero-flash mechanism (its config.gz and
 * kallsyms show no 0f symbols); the chip boots from internal flash. With
 * HX_ZERO_FLASH the resume path ran a bogus FW update and then reset the
 * whole TDDI, corrupting the display after screen-on. Disable it. */
/*#define HX_ZERO_FLASH*/

/*system suspend-chipset power off,
 *oncell chipset need to enable the definition
 */
/* tb8788p1: the stock resume does ic_reset+pin_reset, but on the GSI the
 * boot-time LCD off/on cycle also fires the touch resume and the reset
 * there boot-looped the device. The resume re-applies only the sorting/
 * raw-out config (see himax_chip_common_resume) instead. */
...
/* tb8788p1: part of the zero-flash resume flow; disabled with HX_ZERO_FLASH */
...
#define BOOT_UPGRADE_FWNAME "Himax_firmware_spi.bin"
#if defined(HX_ZERO_FLASH)
...
/* tb8788p1: verified against the stock 2.1.0.6 driver at runtime
 * (dumped its config structs on the stock kernel): the panel is
 * 48 RX / 32 TX with 10-point + pen reporting, and touch_info_size
 * becomes (10 + 2 + 2) * 4 + PEN_INFO_SZ(12) = 68 bytes — the size
 * the stock driver reads/checksums per event. The previous 5-point
 * values (from the donor phone) made touch_info_size 32, so the
 * checksum never matched and no events were reported. */
enum fix_touch_info {
	FIX_HX_RX_NUM = 48,
	FIX_HX_TX_NUM = 32,
	FIX_HX_BT_NUM = 0,
