...
//przie-add-pengzhipeng-20200422-start
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
#include "../../../../misc/mediatek/hardware_info/hardware_info.h"

extern struct hardware_info current_tp_info;
#endif
//przie-add-pengzhipeng-20200422-end
...
void himax_mcu_sense_on(uint8_t FlashMode)
{
	uint8_t tmp_data[DATA_LEN_4];
	int retry = 0;
	int ret = 0;
...
bool himax_mcu_sense_off(bool check_en)
{
	uint8_t cnt = 0;
	uint8_t tmp_data[DATA_LEN_4];
	int ret = 0;
...
static void himax_mcu_read_FW_ver(void)
{
	...
	//przie-add-pengzhipeng-20200422-start
	#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_tp_info.chip,"fw_version:D%2.2XC%2.2X", ic_data->vendor_display_cfg_ver, ic_data->vendor_touch_cfg_ver);
    sprintf(current_tp_info.id,"0x%x",0x83102D);
    strcpy(current_tp_info.vendor,"Himax");
    sprintf(current_tp_info.more,"%d*%d",1560,720);
	#endif
//przie-add-pengzhipeng-20200422-end
#if 0
END:
...
static void hx_ap_notify_fw_sus(int suspend)
{
	...
	Arr4_to_Arr4(addr_tmp, pfw_op->addr_ap_nodfy_fw_sus);

	if (suspend) {
		I("%s,Suspend mode!\n", __func__);
		Arr4_to_Arr4(data_tmp, pfw_op->data_ap_nodfy_fw_sus_en);
	} else {
		I("%s,NonSuspend mode!\n", __func__);
		Arr4_to_Arr4(data_tmp, pfw_op->data_ap_nodfy_fw_sus_dis);
	}
...
/* CORE_FW */
	...
	himax_parse_assign_cmd(fw_addr_ap_nodfy_fw_sus,
		pfw_op->addr_ap_nodfy_fw_sus,
		sizeof(pfw_op->addr_ap_nodfy_fw_sus));
	himax_parse_assign_cmd(fw_data_ap_nodfy_fw_sus_en,
		pfw_op->data_ap_nodfy_fw_sus_en,
		sizeof(pfw_op->data_ap_nodfy_fw_sus_en));
	himax_parse_assign_cmd(fw_data_ap_nodfy_fw_sus_dis,
		pfw_op->data_ap_nodfy_fw_sus_dis,
		sizeof(pfw_op->data_ap_nodfy_fw_sus_dis));
