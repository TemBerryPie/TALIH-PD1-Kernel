...
/* SMWP */
	...
	/* tb8788p1 dynamic trace: checksum outcome (flood-safe counters) */
	g_hxtrace.last_cksum = check_sum_cal & 0xff;

	if (check_sum_cal % 0x100 != 0) {
		atomic_long_inc(&g_hxtrace.cksum_fail);
		I("point data_checksum not match check_sum_cal: 0x%02X",
			check_sum_cal);
		ret_val = HX_CHKSUM_FAIL;
	} else if (zero_cnt == length) {
		atomic_long_inc(&g_hxtrace.cksum_allzero);
		if (ts->use_irq)
			I("[HIMAX TP MSG] All Zero event\n");

		ret_val = HX_CHKSUM_FAIL;
	} else {
		atomic_long_inc(&g_hxtrace.cksum_ok);
...
void himax_ts_work(struct himax_ts_data *ts)
{
	...
	/* tb8788p1 dynamic trace: prove the threaded IRQ handler actually ran.
	 * Atomic counter (printk gets flooded/ratelimited in the IRQ storm). */
	atomic_long_inc(&g_hxtrace.ts_work_enter);
	...
	/* tb8788p1 dynamic trace: which report path was taken */
	if (ts_path >= 0 && ts_path < 4)
		atomic_long_inc(&g_hxtrace.ts_path[ts_path]);
	...
	/* tb8788p1 dynamic trace: operation outcome */
	if (ts_status == HX_TS_GET_DATA_FAIL)
		atomic_long_inc(&g_hxtrace.get_fail);
	else
		atomic_long_inc(&g_hxtrace.op_done);
...
#if defined(HX_ZERO_FLASH)
	/* tb8788p1: the stock driver does NOT force a boot-time firmware
	 * upgrade on this HX83102E (it has no zero-flash path and skips the
	 * update since the FW version matches). Forcing g_boot_upgrade_flag
	 * here skipped power_on_init at probe and ran a broken 0f update
	 * (part_num=0) that left the TDDI touch not scanning. Keep the flag
	 * clear so power_on_init runs and the version check skips the flash,
	 * exactly like the stock driver. */
	g_boot_upgrade_flag = 0;
#else
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
	...
	/* tb8788p1: the lcm suspend powers the TDDI rails down, so at every
	 * real wake the touch FW re-boots from its flash; the stock resume
	 * (reload handshake only) leaves the scan off and the runtime config
	 * unset, which kills the touch after a few wake cycles. Re-run the
	 * full probe-time init (raw_out/sorting reset + reload wait +
	 * sense_on) so the wake state matches the boot state. */
	if (g_core_fp.fp_power_on_init != NULL)
		g_core_fp.fp_power_on_init();
	else if (g_core_fp.fp_resume_ic_action != NULL)
