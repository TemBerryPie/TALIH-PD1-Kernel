...
int tpd_get_gpio_info(struct platform_device *pdev)
{
	...
	/* tb8788p1: the touch SPI pins (miso/mosi/cs/clk) must be switched to
	 * their SPI function (_h pin groups) before the touch can be accessed
	 * over SPI; the states live in the elink,touch node's pinctrl. */
	{
		static const char * const spi_state_names[] = {
			"spi_mi_h", "spi_cs_h", "spi_mo_h", "spi_clk_h",
		};
		int k;

		for (k = 0; k < ARRAY_SIZE(spi_state_names); k++) {
			struct pinctrl_state *st;

			st = pinctrl_lookup_state(pinctrl1, spi_state_names[k]);
			if (IS_ERR(st)) {
				TPD_DMESG("no pinctrl state %s\n",
					  spi_state_names[k]);
				continue;
			}
			if (pinctrl_select_state(pinctrl1, st))
				TPD_DMESG("failed to select %s\n",
					  spi_state_names[k]);
		}
	}
	TPD_DEBUG("[tpd%d] mt_tpd_pinctrl----------\n", pdev->id);
